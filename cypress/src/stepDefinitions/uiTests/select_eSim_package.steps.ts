...
Given(
  /^I search and select "([^"]*)" under "([^"]*)" region$/,
  function (coverage: string, region: string) {
    ...
    cy.get('ul.countries-list.position-absolute')
      ...
      .within(() => {
        switch (region) {
          ...
          case 'Regional':
            cy.get('li')
              .contains(commonFunctions.returnRegionalValue(coverage))
              .click();
            break;
          default:
...
Given(
  /^I select the e-sim package that have the following details$/,
  function (dataTable: DataTable) {
    const dataTableObject =
      commonFunctions.convertDataTableIntoObject(dataTable);
    const coverageName = commonFunctions.lowercaseFirstCharOfEachWord(
      dataTableObject.Coverage,
    );
    const urlExtension = commonFunctions.returnUrlExtension(
      coverageName,
      dataTableObject.area,
    );
    const eSimPrice = commonFunctions.formatCurrency(
      dataTableObject.eSimValue,
      dataTableObject.eSimCurrency,
    );
...
Then(/^I verify the e-sim package I selected$/, function () {
  ...
  const simValue = commonFunctions.formatCurrency(
    retrievedDataTableObject.eSimValue,
    retrievedDataTableObject.eSimCurrency,
  );
  selectEsimPackages.assertSimPackageDetails(
    simValue,
    retrievedDataTableObject,
  );
});
